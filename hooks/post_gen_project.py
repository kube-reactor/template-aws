...
def save_secret_environment(environment):
    secret_template_file = os.path.join("env", environment, "secret.example.sh")
    secret_env_file = os.path.join("env", environment, "secret.sh")
    secret_script = ""
    ...
    with open(secret_env_file, "w+") as file:
        file.writelines(
            secret_script.format(
                secret_key=generate_random_string(
                    10, using_ascii_letters=True, using_digits=False
                ),
                long_secret_key=generate_random_string(
                    20, using_ascii_letters=True, using_digits=False
                ),
                password=generate_random_string(
                    10, using_ascii_letters=True, using_digits=True
                ),
                strong_password=generate_random_string(
                    15, using_ascii_letters=True, using_digits=True
                ),
            )
        )
...
def main():
    for environment in ["local", "prod"]:
        save_secret_environment(environment)
